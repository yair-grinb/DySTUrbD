...
def create_social_network(agents, households, build):
    ...
    fs = gv.bldgs[:, 4] # get floor space
    dists = spatial.distance_matrix(gv.bldgs[:, 6:8], gv.bldgs[:, 6:8])**beta # matrix of squared distances between buildings
    ...
    for i in range(len(gv.bldgs)):
        scores[i, i] = 0 # attractivity of a building to itself = 0
    rows_sum = scores.sum(axis=1).reshape((len(gv.bldgs), 1)) # sum of each row for computing probability scores
    ...
    agent_hh = (gv.households[:, 0][:, None] == gv.indivs[:, 1]).argmax(axis=0)
    hh_income = gv.households[agent_hh, 2] # household incomes
    income_dist = np.abs(np.subtract(hh_income, hh_income.reshape((len(gv.indivs), 1)))) # matrix of difference in incomes
    ...
    age_dist = np.abs(np.subtract(gv.indivs[:, 4], gv.indivs[:, 4].reshape((len(gv.indivs), 1)))) # matrix of age difference
    age_dist = 1 - age_dist / np.max(age_dist) # agents of similar ages have higher probabilities
    hh_home = gv.households[agent_hh, 1] # find homes of households
    agent_homes = (gv.bldgs[:, 0][:, None] == hh_home).argmax(axis=0) # find homes of agent
    agent_dists = spatial.distance_matrix(gv.bldgs[agent_homes, 6:8], gv.bldgs[agent_homes, 6:8]) # matrix of distances between agents
    ...
    agents_prob[agent_hh == agent_hh.reshape((len(gv.indivs), 1))] = 1 # identify agents from the same household and set prob to 1
    ...
    edges = [(gv.bldgs[b,0], gv.bldgs[b1,0], -np.log(bld_prob[b, b1])) 
             for b in range(len(gv.bldgs)) for b1 in range(len(gv.bldgs)) if bld_prob[b, b1] > b_min_prob and  b != b1]
    # create edges with probability weight=1 between agents and their homes
    edges.extend([(gv.indivs[a, 0], gv.bldgs[agent_homes[a], 0], np.log(1)) for a in range(len(gv.indivs))])
    # create edges with probability weight=1 between agents and their workplaces
    edges.extend([(gv.indivs[a, 0], gv.indivs[a, 12], np.log(1)) for a in range(len(gv.indivs)) 
                  if ~np.isnan(gv.indivs[a, 12])])
    edges.extend([(gv.indivs[a, 0], gv.indivs[a, 18], np.log(1)) for a in range(len(gv.indivs)) 
                  if ~np.isnan(gv.indivs[a, 18])])
    ...
    for i in range(len(links[0])):
        if links[0][i] != links[1][i]:
            G.add_weighted_edges_from([(gv.indivs[links[0][i], 0],
                                        gv.indivs[links[1][i], 0],
...
def create_routines(agents_reg, calculate_distances=True):
    ...
    for n in range(len(gv.agents_reg)):
        a = gv.agents_reg[n, 0]
        current_position = gv.agents_reg[n, 1]
